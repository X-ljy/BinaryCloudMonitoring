 ...
 * @date 2019/6/29
 */
public class BinaryAgentNetworkCardInfoProducer {
    static Producer<String, String> producer = new KafkaProducer<>(KafkaInfo.getProperties());

    public static String runBinaryAgentNetworkCardInfoProducer(UserInfoBean userInfoBean){

        JSONArray serverIdArray = new JSONArray(userInfoBean.getHostID());
        for (int i =0 ;i < serverIdArray.length(); i++){
            String res = ServerInfo.getBinaryAgentNetworkCardInfo(IpMap.ipList.get(serverIdArray.getString(i)),"6666");
            if (res.equals("error")){
                continue;
            }else {
                producer.send(new ProducerRecord<>(TopicList.binaryAgentNetworkCardInfo,serverIdArray.getString(i),res));
        }
        }
        return "success";
    }
