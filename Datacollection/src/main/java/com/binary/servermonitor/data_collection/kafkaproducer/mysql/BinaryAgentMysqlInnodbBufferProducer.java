 ...
 * @date 2019/7/2
 */
public class BinaryAgentMysqlInnodbBufferProducer {

    static Producer<String, String> producer = new KafkaProducer<>(KafkaInfo.getProperties());

    public static String runBinaryAgentMysqlInnodbBufferProducer(UserInfoBean userInfoBean){

        JSONArray serverIdArray = new JSONArray(userInfoBean.getHostID());
        for (int i =0 ;i < serverIdArray.length(); i++){
            String res = ServerInfo.getMysqlInnodbBuffer(IpMap.ipList.get(serverIdArray.getString(i)),"6666");
            if (res.equals("error")){
                continue;
            }else {
                producer.send(new ProducerRecord<>(TopicList.binaryAgentMysqlInnodbBuffer,serverIdArray.getString(i),res));
            }
        }
        return "success";
    }
