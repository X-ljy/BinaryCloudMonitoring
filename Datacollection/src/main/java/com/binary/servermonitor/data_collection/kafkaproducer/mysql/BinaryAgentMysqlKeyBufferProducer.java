 ...
 * @date 2019/7/2
 */
public class BinaryAgentMysqlKeyBufferProducer {
    static Producer<String, String> producer = new KafkaProducer<>(KafkaInfo.getProperties());

    public static String runBinaryAgentMysqlKeyBufferProducer(UserInfoBean userInfoBean){

        JSONArray serverIdArray = new JSONArray(userInfoBean.getHostID());
        for (int i =0 ;i < serverIdArray.length(); i++){
            String res = ServerInfo.getMysqlKeyBuffer(IpMap.ipList.get(serverIdArray.getString(i)),"6666");
            if (res.equals("error")){
                continue;
            }else {
                producer.send(new ProducerRecord<>(TopicList.binaryAgentMysqlKeyBuffer,serverIdArray.getString(i),res));
            }
        }
        return "success";
    }
