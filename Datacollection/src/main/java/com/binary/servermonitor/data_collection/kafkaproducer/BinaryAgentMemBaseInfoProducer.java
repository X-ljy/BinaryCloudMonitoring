 ...
 * @date 2019/6/29
 */
public class BinaryAgentMemBaseInfoProducer {
    static Producer<String, String> producer = new KafkaProducer<>(KafkaInfo.getProperties());

    public static String runBinaryAgentMemBaseInfoProducer(UserInfoBean userInfoBean){

        JSONArray serverIdArray = new JSONArray(userInfoBean.getHostID());
        for (int i =0 ;i < serverIdArray.length(); i++){
            String res = ServerInfo.getBinaryAgentMemBaseInfo(IpMap.ipList.get(serverIdArray.getString(i)),"6666");
            if (res.equals("error")){
                continue;
            }else {
                producer.send(new ProducerRecord<>(TopicList.binaryAgentMemBaseInfo,serverIdArray.getString(i),res));
            }
        }
        return "success";
    }
