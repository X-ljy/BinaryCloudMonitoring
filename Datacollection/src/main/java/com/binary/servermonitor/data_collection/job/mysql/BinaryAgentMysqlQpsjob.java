 ...
 * @date 2019/7/2
 */
public class BinaryAgentMysqlQpsjob implements Job {
    private static  org.slf4j.Logger logger = LoggerFactory.getLogger(BinaryAgentMysqlQpsjob.class);
    private static UserInfoBean userInfoBean;
    public static UserInfoBean getUserInfoBean() {
        return userInfoBean;
    }
    public static void setUserInfoBean(UserInfoBean userInfoBean) {
        BinaryAgentMysqlQpsjob.userInfoBean = userInfoBean;
    }
    @Override
    public void execute(JobExecutionContext jobExecutionContext) {
        try {
            BinaryAgentMysqlQpsProducer.runBinaryAgentMysqlQpsProducer(getUserInfoBean());
        }catch (Exception e){
            logger.error("BinaryAgentMysqlQpsjob : " + e.getMessage());
        }
    }
}