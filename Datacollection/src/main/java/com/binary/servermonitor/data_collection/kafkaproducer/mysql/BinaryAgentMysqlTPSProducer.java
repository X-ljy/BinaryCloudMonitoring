 ...
 * @date 2019/7/2
 */
public class BinaryAgentMysqlTPSProducer {

    static Producer<String, String> producer = new KafkaProducer<>(KafkaInfo.getProperties());

    public static String runBinaryAgentMysqlTPSProducer(UserInfoBean userInfoBean){

        JSONArray serverIdArray = new JSONArray(userInfoBean.getHostID());
        for (int i =0 ;i < serverIdArray.length(); i++){
            String res = ServerInfo.getMysqlTPS(IpMap.ipList.get(serverIdArray.getString(i)),"6666");
            if (res.equals("error")){
                continue;
            }else {
                producer.send(new ProducerRecord<>(TopicList.binaryAgentMysqlTPS,serverIdArray.getString(i),res));
            }
        }
        return "success";
    }
